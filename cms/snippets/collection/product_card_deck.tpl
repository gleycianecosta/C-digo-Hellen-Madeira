...
<div class="collection-grid-card product-card-cards position-relative template-cards h-100 flex{% if not product.stock > 0 %} out-of-stock{% endif%}" direction="column" align-items="center" justify-content="between" data-id="{{product.id}}">
  <a href="{{product.url}}" title="{{product.name|raw}}" class="flex product-link w-100 h-100 position-relative" align-items="center" justify-space="between" direction="{{card_direction}}">
    ...
    <div class="position-absolute list-only flex w-100">
      <div class="product-card-deck-title flex text-center" justify-content="center"> 
        <span>{{product.name|raw}}</span>
      </div>
    </div>
    {% else %}
    <div class="position-absolute product-card-deck-title flex text-center grid-only w-100" justify-content="center">
      <span>{{product.name|raw}}</span>
    </div>
    {% endif %}
    <figure class="flex product-card-deck-thumbnail text-center w-100 position-relative" direction="column" align-items="center">
  ...
  <div class="cards-foot flex w-100 h-100" justify-content="space-between" align-items="center">
   {% if not product.show_price %}
        <div class="product-card-list-price-blocked flex" gap="10" direction="row" wrap="false">
        ...
        {% else %}
    <div class="product-card-deck-price">
      <div class="currentPrice flex" data-element="price" direction="column">
        {% include 'snippets/collection/product-card-price.tpl' %}
      </div>
    </div>
    {% endif %}
    {% if product.stock > 0 and product.price.list > 0 and product.show_price %}
    <div class="product-card-deck-action h-100 flex" align-items="center">
      {{ include('snippets/collection/product-card-action.tpl', {actionType:cardType})}}
    </div>
    {% elseif product.price.list > 0 and product.show_price %}
